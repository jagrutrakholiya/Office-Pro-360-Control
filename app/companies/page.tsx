      ...
      <div className="section-header">
        <div className="section-actions">
          <div>
            ...
            <p className="section-subtitle">Manage companies, plans, and subscriptions</p>
          </div>
          <button 
        ...
        <div className="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-slate-100">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-bold text-slate-900">All Companies</h3>
              <span className="badge badge-pending">{companies.length} total</span>
            </div>
            <div className="flex items-center gap-2">
              <input
                type="search"
          ...
          <table className="w-full">
            <thead className="table-header">
              <tr>
                ...
                <th className="text-left px-6 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider">Services</th>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <code className="text-xs bg-slate-100 px-2 py-0.5 rounded text-slate-600">{c.code}</code>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
            ...
            <div key={c._id} className="mobile-card">
              <div className="mobile-card-header">
                ...
                <span className={`badge ${c.status === 'active' ? 'badge-active' : c.status === 'view_only' ? 'badge-view-only' : 'badge-suspended'}`}>
                  <div className={`status-dot ${c.status === 'active' ? 'status-dot-active' : c.status === 'suspended' ? 'status-dot-suspended' : 'status-dot-pending'}`}></div>
                  {c.status}
                </span>
              </div>
              <div className="mobile-card-content">
                ...
                <div className="mobile-card-row">
                  ...
                  <span className="mobile-card-value">
                    {c.features ? Object.values(c.features).filter(Boolean).length : 0} features
                  </span>
                </div>
                ...
                {Object.entries(categorizedServices).map(([category, services]) => (
                  <div key={category} className="space-y-3">
                    <div className="flex items-center gap-2 pb-2 border-b border-slate-200">
                      ...
                      <h5 className="text-sm font-bold text-slate-900">{category}</h5>
