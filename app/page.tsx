...
export default function DashboardPage() {
  ...
  const StatCard = ({ title, value, subtitle, icon, gradient, onClick }: {
    ...
    <div 
      onClick={onClick}
      className={`card-gradient ${gradient} ${onClick ? 'card-interactive' : ''} group relative overflow-hidden`}
    >
      <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      <div className="relative z-10 flex items-start justify-between">
        <div className="flex-1 min-w-0">
          <div className="text-sm text-white/80 mb-2 font-medium">{title}</div>
          <div className="text-3xl sm:text-4xl font-bold mb-2 text-white">{value}</div>
          {subtitle && <div className="text-xs text-white/70 leading-relaxed">{subtitle}</div>}
        </div>
          ...
          <div className="flex flex-col xs:flex-row items-start xs:items-center gap-3 xs:gap-4">
            <div className="flex items-center gap-2 text-sm text-slate-600">
              <input
                ...
                id="auto-refresh"
              />
              <label htmlFor="auto-refresh" className="cursor-pointer font-medium">
                Auto-refresh (30s)
              </label>
            </div>
            {lastUpdated && (
      ...
      <div className="responsive-grid-3 mb-8">
        <div className="card">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-slate-900">Company Status</h3>
          </div>
          <div className="space-y-4">
            <div className="flex items-center justify-between p-3 bg-green-50 rounded-xl">
              ...
              <span className="text-2xl font-bold text-green-600">{stats?.companies.active || 0}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-red-50 rounded-xl">
              ...
              <span className="text-2xl font-bold text-red-600">{stats?.companies.suspended || 0}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-blue-50 rounded-xl">
              ...
              <span className="text-2xl font-bold text-blue-600">{stats?.companies.thisMonth || 0}</span>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-slate-900">Plans Overview</h3>
          </div>
          <div className="space-y-4">
            <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
              <span className="text-slate-700 font-medium">Total Plans</span>
              <span className="text-2xl font-bold text-slate-900">{stats?.plans.total || 0}</span>
            </div>
            {stats?.earnings.byPlan && Object.entries(stats.earnings.byPlan).map(([plan, data]) => (
              <div key={plan} className="flex items-center justify-between p-3 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl">
                <span className="text-slate-700 font-medium capitalize">{plan}</span>
                <div className="text-right">
                  <div className="text-lg font-bold text-slate-900">{data.count} companies</div>
                  <div className="text-sm text-slate-500">{formatCurrency(data.mrr)}/mo</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="card">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xl font-bold text-slate-900">Recent Companies</h3>
          </div>
          <button
            onClick={() => router.push('/companies')}
              ...
              <table className="w-full">
                <thead className="table-header">
                  <tr>
                    ...
                    <th className="text-left px-6 py-4 text-xs font-semibold text-slate-600 uppercase tracking-wider">Code</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {stats.recentCompanies.map((company) => (
                    <tr key={company._id} className="table-row">
                      ...
                      <td className="px-6 py-4">
                        <code className="text-sm bg-slate-100 px-2 py-1 rounded text-slate-600">{company.code}</code>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm text-slate-700 capitalize font-medium">{company.plan}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`badge ${
                          ...
                          <div className={`status-dot ${
                            company.status === 'active' ? 'status-dot-active' : 
                            company.status === 'suspended' ? 'status-dot-suspended' : 
                            'status-dot-pending'
                          }`}></div>
                          {company.status}
                        </span>
                ...
                <div key={company._id} className="mobile-card">
                  ...
                  <div className="mobile-card-content">
                    <div className="mobile-card-row">
                      ...
                      <code className="text-xs bg-slate-100 px-2 py-1 rounded text-slate-600">{company.code}</code>
                    </div>
                    <div className="mobile-card-row">
                      <span className="mobile-card-label">Plan</span>
                      <span className="mobile-card-value capitalize">{company.plan}</span>
                    </div>
                    <div className="mobile-card-row">
                      <span className="mobile-card-label">Created</span>
                      <span className="mobile-card-value">{formatDate(company.createdAt)}</span>
                    </div>
                  </div>
                </div>
              ))}
